...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userDetailResponse = await fetch("http://localhost:1337/api/users/me?populate=*", {
        headers: {
      ...
      await Swal.fire({
        ...
        html: userDetailData.profilePicture && userDetailData.profilePicture[0]
          ? `
  ...
  return (
    ...
    >
      <h1 className="text-2xl font-light tracking-wider text-center mb-2 mt-8 pt-8">LOGIN</h1>
...
<div className=" mt-6 pt-3">
          <button
            ...
            style={{ maxWidth: '170px', height: '35px' , margin: '0 auto' , borderRadius: '15px' , backgroundColor: '#FFE4B5' , opacity: 0.9, transition: 'background-color 0.4s ease'}}
            onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#FFDEAD'}
            onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#FFE4B5'}
          >
            Sign in
          </button>
        </div>
      ...
      <p className="mt-4 text-center text-sm text-gray-500">
        ...
        <button onClick={handleSignUp} className="font-semibold text-indigo-600 hover:text-indigo-500">
          Sign up
        </button>
      </p>
    </div>
