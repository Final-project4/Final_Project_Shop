...
export default factories.createCoreController('api::order.order', ({ strapi }) => ({
  ...
  async findOne(ctx) {
    ...
    try {
      ...
      const order = await strapi.db.query('api::order.order').findOne({
        where: { id: orderId },
        populate: ['order-items', 'customer'], // ดึงข้อมูล order-items และ customer ถ้ามี
      });
  ...
  async update(ctx) {
    ...
    const { step, total_price } = ctx.request.body.data || ctx.request.body; // รองรับทั้งสองรูปแบบ
  
    console.log('Parsed Data:', { step, total_price }); // Debug ค่าหลังจาก parse
